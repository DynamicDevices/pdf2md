...
if [[ "$PAGE_TEST_MODE" != "1" ]]; then
  for r in "$OUT_TEXT/README.md" "$OUT_PDF2MD_AUTO/README.md" "$OUT_PDF2MD_ALWAYS/README.md"; do
    if [[ "$r" == "$OUT_PDF2MD_ALWAYS/README.md" && "$ALSO_ALWAYS" != "1" ]]; then
      continue
    fi
    if [[ ! -f "$r" ]]; then
      cat > "$r" <<'EOF'
...
## Notes
- Compare fidelity vs original PDF here.
EOF
    fi
  done
...
if [[ "$PAGE_TEST_MODE" != "1" ]]; then
cat >> "$OUT_PDF2MD_AUTO/README.md" <<EOF
...
if [[ "$ALSO_ALWAYS" == "1" ]]; then
if [[ "$PAGE_TEST_MODE" != "1" ]]; then
cat >> "$OUT_PDF2MD_ALWAYS/README.md" <<EOF
